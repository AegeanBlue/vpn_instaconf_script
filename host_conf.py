...
def main(): 
    ...
    subprocess.run('sudo apt install visudo', text=True, shell=True)
    
    print('Checking for Curl...\n ')
    subprocess.run('sudo apt install curl', text=True, shell=True)
    
    print('Checking for Iptables...\n ')
    subprocess.run('sudo apt install iptables', text=True, shell=True)

    user_input_hostname = input('Change hostname (leave empty to keep default value): ')
    if user_input_hostname:
        subprocess.run(f'sudo hostname {user_input_hostname}', text=True, shell=True)
    
    user_input_timezone = input('Change timezone (leave empty to keep default value): ')
    if user_input_timezone:
        subprocess.run(f'sudo timedatectl set-timezone {user_input_timezone}', text=True, shell=True)

    user_input_ssh_port = input('change ssh port (leave empty to keep default value): ')
    ...
    user_input_new_user = input('New user (leave empty to skip this step): ')
